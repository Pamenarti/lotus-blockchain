...
from lotus.ssl.create_ssl import generate_ca_signed_cert, get_lotus_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    all_targets = []
    stop_searching_for_farmer = "lotus_target_address" not in config["farmer"]
    stop_searching_for_pool = "lotus_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("lotus_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("lotus_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "lotus_target_address" not in config["farmer"]:
        print(f"Setting the lotus destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["lotus_target_address"] = all_targets[0]
    elif config["farmer"]["lotus_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['lotus_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "lotus_target_address" not in config["pool"]:
        print(f"Setting the lotus destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["lotus_target_address"] = all_targets[0]
    elif config["pool"]["lotus_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['lotus_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    lotus_ca_crt_path.write_bytes(lotus_ca_crt)
    lotus_ca_key_path.write_bytes(lotus_ca_key)
