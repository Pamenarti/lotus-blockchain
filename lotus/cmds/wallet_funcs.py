...
async def print_balances(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    for summary in summaries_response:
        ...
        if typ != "STANDARD_WALLET":
            ...
            print(
                f"   -Total Balance: {balances['confirmed_wallet_balance']/units['lotus']} {address_prefix} "
                f"({balances['confirmed_wallet_balance']} graviton)"
            )
            print(
                f"   -Pending Total Balance: {balances['unconfirmed_wallet_balance']/units['lotus']} {address_prefix} "
                f"({balances['unconfirmed_wallet_balance']} graviton)"
            )
            print(
                f"   -Spendable: {balances['spendable_balance']/units['lotus']} {address_prefix} "
                f"({balances['spendable_balance']} graviton)"
            )
    ...
    if log_in_response["success"] is False:
        if log_in_response["error"] == "not_initialized":
            ...
            if "backup_path" in log_in_response:
                ...
                print(f"Backup file from backup.lotuscoin.org downloaded and written to: {path}")
