...
from lotus.plotting.create_plots import create_plots
from lotus.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
...
from lotus.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
class BlockTools:
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "lotus")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("lotus_ROOT", "~/.lotus-blockchain/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
